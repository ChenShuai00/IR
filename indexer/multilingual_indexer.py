...
#             if token not in self.stopwords[language]
#         ]
#
#         # 计算词频
...
#         """从原始数据构建索引"""
#         print(f"发现JSON文件数量: {len(list(Path(raw_data_dir).glob('*.json')))}")  # 检查文件数量
#         for i, filepath in enumerate(Path(raw_data_dir).glob('*.json')):
#             if i < 3:  # 打印前3个文件内容样本
#                 with open(filepath, 'r', encoding='utf-8') as f:
#                     print(f"样本 {i}: {json.load(f).keys()}")  # 检查字段结构
#
#         doc_id = 0
#         total_length = 0
#
#         for filepath in Path(raw_data_dir).glob('*.json'):
#             with open(filepath, 'r', encoding='utf-8') as f:
...
#         # 计算平均文档长度
#         if doc_id > 0:
#             self.avg_doc_length = total_length / doc_id
#
#         self.save_index()
#
#     def save_index(self):
...
#     AI_news_indexer = MultilingualIndexer()
#     AI_news_indexer.build_from_raw_data('../data/raw')
import json
from collections import defaultdict
from pathlib import Path
...
if __name__ == "__main__":
    try:
        ...
        indexer.build_from_raw_data('../data/raw')  # 修改为您的实际路径
    except Exception as e:
